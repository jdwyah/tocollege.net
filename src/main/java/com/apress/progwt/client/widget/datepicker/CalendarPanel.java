 ...
 * selectable items. Clicking on any weekday makes the selection and
 * the date is displayed in the textbox. All the weekends are grayed 
 * out and doesnot allow selection
 */
public class CalendarPanel extends AbsolutePanel {
	 ...
	 * buildMonthHeader
	 * 
	 * Displays the header of the month with all the 
	 ...
	 * made selectable while weekends are grayed out and cannot be selected
	 */
	protected void buildMonthBody(Date date, FlexTable monthBody){
		int startWeekDay = DateUtil.getStartWeekDay(date);
		int numDays = DateUtil.getNumDaysInMonth(date.getMonth(), DateUtil.isLeapYear(date));
		int dayCount=0;
	...
	protected class DateClickListener implements TableListener{
		...
		public void onCellClicked(SourcesTableEvents sender, int row, int col) {
			...
			if (null == cellValue || "null".equals(cellValue) || cellValue.trim().length() == 0){
				return;
			}
			Date currentDate = this.calPanel.datePicker.getCurrentDate();
			/*
			 String dateStr = DateFormatter.formatDate(
			 new Date(	currentDate.getYear(), 
	 ...
	 * isSelecteDate
	 * 
	 * Checks whether or not the date being printed is the
