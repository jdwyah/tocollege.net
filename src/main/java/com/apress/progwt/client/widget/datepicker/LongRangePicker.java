...
//		datePicker.hide();
	...
	private class YearLabel extends RangeLabel{
		...
		public void setToDate(Date d) {
			...
			int intervalStart = (getYear(d) / span)*span;//int division floors
			int selected = (getYear(d) / (span/10))*(span/10);
			
			//System.out.println("getYear(d) "+getYear(d)+" ints "+intervalStart+" sel "+selected);
	...
	private class Clicker extends Label implements ClickListener{
		...
		/**
		 * move the range down one section. don't just subtract though,
		 * or 2000 - 10 = 1900 we'd prefer 2000 - 10 to be 1990.
		 */
		public void onClick(Widget sender) {
			...
			if(span > 1){
				...
				//so for 1950, interval start == 1900, then + (-100/10) = 1890
