...
/**
 * A sample toolbar for use with {@link RichTextArea}. It provides a
 * simple UI for all rich text formatting, dynamically displayed only for
...
public class RichTextToolbar extends Composite {
     ...
     * image bundle allows all of these images to be packed into a single
     * image, which saves a lot of HTTP requests, drastically improving
     * startup time.
     */
    ...
    /**
     * This {@link Constants} interface is used to make the toolbar's
     * strings internationalizable.
     */
    ...
    /**
     * We use an inner EventListener class to avoid exposing event methods
     * on the RichTextToolbar itself.
     */
    private class EventListener implements ClickListener, ChangeListener,
        ...
        public void onChange(Widget sender) {
            if (sender == backColors) {
                ...
                basic.setFontName(fonts
                        .getValue(fonts.getSelectedIndex()));
        ...
        public void onClick(Widget sender) {
            if (sender == bold) {
                ...
                String url = Window.prompt("Enter an image URL:",
                        "http://");
                if (url != null) {
                ...
                // toolbar status.
                // This will catch any cases where the user moves the
                // cursur using the
                // keyboard, or uses one of the browser's built-in
                // keyboard shortcuts.
                updateStatus();
            }
        }
    ...
    public RichTextToolbar(RichTextArea richText) {
        ...
        if (basic != null) {
            ...
            topPanel.add(italic = createToggleButton(images.italic(),
                    strings.italic()));
            topPanel.add(underline = createToggleButton(images
                    .underline(), strings.underline()));
            topPanel.add(subscript = createToggleButton(images
                    .subscript(), strings.subscript()));
            topPanel.add(superscript = createToggleButton(images
                    .superscript(), strings.superscript()));
            topPanel.add(justifyLeft = createPushButton(images
                    .justifyLeft(), strings.justifyLeft()));
            topPanel.add(justifyCenter = createPushButton(images
                    .justifyCenter(), strings.justifyCenter()));
            topPanel.add(justifyRight = createPushButton(images
                    .justifyRight(), strings.justifyRight()));
        }
        ...
        if (extended != null) {
            ...
            topPanel.add(outdent = createPushButton(images.outdent(),
                    strings.outdent()));
            topPanel
                    .add(hr = createPushButton(images.hr(), strings.hr()));
            topPanel
                    .add(ol = createPushButton(images.ol(), strings.ol()));
            topPanel
                    .add(ul = createPushButton(images.ul(), strings.ul()));
            topPanel.add(insertImage = createPushButton(images
                    .insertImage(), strings.insertImage()));
            topPanel.add(createLink = createPushButton(images
                    .createLink(), strings.createLink()));
            topPanel.add(removeLink = createPushButton(images
                    .removeLink(), strings.removeLink()));
            topPanel.add(removeFormat = createPushButton(images
                    .removeFormat(), strings.removeFormat()));
        }
